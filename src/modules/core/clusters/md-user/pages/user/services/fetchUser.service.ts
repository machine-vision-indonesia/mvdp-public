...
import {
  ...
  GetTableUsersResponse,
  GetWorkCentersResponse,
  Params,
  PropsTable
} from '../types/ManageUserPage.types'
...
import { PropsTableAsyncCollapsed } from '@/components/templates/table-async-collapse/types/TableAsyncCollapsed.type'
...
export const fetchTableUserCollapsed: PropsTableAsyncCollapsed['dataFetchService'] = params => {
  const queryParams = {
    ...params,
    limits: -1,
    fields: [
          ...
          _or: [
            {
              user: {
                profile: {
                  full_name: {
                    _icontains: params?.filterResult?.resultController?.['profile.full_name'] ?? undefined
                  }
                }
              }
            }
